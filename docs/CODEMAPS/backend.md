...
| `fetchRuns/`           | FetchRun lifecycle, stale recovery, ordered receipts, private Job intake, attempt/dispatch locks, and the shared `fetch-run-commit/v1` transaction seam                                                                   | `fetchRun.ts`, `fetchRunJobIntake.ts`, `dispatchGithubFetchRun.ts`, `fetchRunStale.ts`, `fetchRunLifecycleLock.ts`, `triggerClaim.ts`                                                                                                                                                                                                                                                             |
...
floor), and the FetchRun module's Job intake implementation.
