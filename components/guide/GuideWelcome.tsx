...
function WelcomeActions({ onStart, onSkip }: GuideWelcomeProps) {
  const t = useTranslations("guide");
  return (
    <div className="mt-6 flex flex-col gap-2">
      ...
      <button
        ...
        onClick={onSkip}
        className={cn(
          "inline-flex items-center justify-center text-xs font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 motion-reduce:transition-none",
...
export function GuideWelcome({ onStart, onSkip }: GuideWelcomeProps) {
  const t = useTranslations("guide");
  return (
    <div className="flex flex-1 flex-col overflow-y-auto px-5 pb-6 pt-7 text-center">
      ...
      <WelcomeFeatures />
      <WelcomeActions onStart={onStart} onSkip={onSkip} />
    </div>
  );
}
