...
**An end-to-end job-search workstation.** Fetch roles, triage them, tailor a custom resume and cover letter for each JD with a local AI agent, and export production-grade PDFs — without copy-paste.
...
![Prisma Models](https://img.shields.io/badge/Prisma_Models-33-7c3aed)
...
runtime** bound to `127.0.0.1`, driven by the [Joblit Runner](./tools/runner/)
— a dependency-free Node process that speaks the same public API any external
agent uses. A job description and a candidate's full work history are among the
most sensitive documents a person owns, so neither ever leaves the machine.
There is no server-side model call, and the Hermes key is never stored
server-side: the Runner reads it locally and refuses to send it anywhere but
loopback.
...
The product differentiator is **where the AI runs**: tailoring happens on your own machine, with your own model, driven by a local Runner you start and stop. Joblit stores the work; it never sees your model key.
...
prompt, scores it through Hermes, and posts the verdicts back. A batch is never
left leased: it imports, or it is marked failed, or its claim is released.
...
Materials land as drafts. Nothing is submitted anywhere, and nothing is sent
without your review.
...
- **Agent tokens are revocable and stored hashed.** A presented token is never
  rescued by a session cookie, so revoking one stops the Runner immediately.
...
`withAgentRoute` accepts either a Bearer agent token or a browser session and
produces the same identity, so the batch protocol, the fit queue and the
tailoring endpoints serve humans and agents through one seam. Tokens are issued
at `/agent`; that endpoint is session-only, because minting a token from a token
would let a leaked credential outlive its revocation.
...
flowchart LR
  ...
  HERMES --> GPT[GPT-5.6 on your machine]
...
app/
  ...
  (app)/              Protected workspace
    ...
    agent/            Runner setup + agent tokens
  api/                Backend routes
    agent-tokens/     Issue and revoke agent tokens (session-only)
...
issue an agent token from `/agent`, then:
...
| `APP_ENC_KEY` | Agent token encryption key (base64) |
