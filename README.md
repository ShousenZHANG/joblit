...
**Fit scan.** Leases a batch of unscored jobs, asks Joblit for the triage
prompt, scores it through Hermes, and posts the verdicts back. The prompt has a
content-addressed 64-hex `issueKey` derived from the sorted Job ids and prompt
snapshot. `FitBatchImportReceipt` commits that issue's validated settlement in
the same transaction as the Job scores, so an identical retry returns the
original result. A definite failure is marked or released; an unknown import
outcome keeps Hermes state for exact replay. Startup checks the durable receipt
before clearing a completed issue. Stop cancels pending/claimed rows at the
database queue boundary; a late local result cannot import through its stale
claim, and the next explicit scan may retry it.
...
accepted, discards only a terminal obsolete tailoring result, and otherwise
preserves the exact active attempt. Atomic compare-and-set makes same-session
Hermes starts single-flight across local Runner processes. The state file
contains opaque ids, hashes, a repair cursor, and non-secret operation identity
— never prompts, model output, feedback text, or credentials.
