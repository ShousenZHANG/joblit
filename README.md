...
![API Handlers](https://img.shields.io/badge/API_Handlers-72-0284c7)
![Test Files](https://img.shields.io/badge/Test_Files-200-65a30d)
![Prisma Models](https://img.shields.io/badge/Prisma_Models-19-7c3aed)
![Server Modules](https://img.shields.io/badge/Server_Modules-71-334155)
...
> **Free & invite-only.** Joblit is free — no paywalls, no tiers. While it scales it's invite-only: request access from the landing page with your email, an admin approves it, and you sign in with that email's Google or GitHub account.
...
- **Invite-only gate** — the NextAuth `signIn` callback admits only configured admins, emails on the approved allowlist, and already-existing users; everyone else is routed to a clear "request access" page rather than a dead-end OAuth error
- **Apply flow** — visitors request access by email (`POST /api/access-requests`, rate-limited, no user enumeration); already-approved emails are sent straight to sign-in
- **Admin console** at `/admin/access` — gated by `ADMIN_EMAILS`; review pending requests and one-click approve / reject
- The OAuth callback is the real boundary: an approved email still has to sign in with that account, so the gate can't be bypassed by guessing the page
...
app/
  ...
  (app)/              Protected workspace
    ...
    admin/            Admin-only access-request console
  api/                Backend routes
    ext/              Extension-specific APIs
    admin/            Import + access-request review (admin/secret-gated)
chrome-extension/
  src/
...
lib/
  ...
  server/             Domain services (AI, PDF, prompts, access, persistence, observability)
