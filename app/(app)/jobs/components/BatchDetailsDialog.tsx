...
import type { ApplicationReviewLoad } from "../hooks/useTailorReviewController";
...
export function BatchDetailsDialog({
  ...
  onCancel: () => void;
  onRetryFailed: () => void;
  reviewLoading: ApplicationReviewLoad | null;
      ...
      >
        ...
        <div className="space-y-5 p-5">
          ...
          {reviewError ? (
            <p
              role="alert"
              className="flex items-start gap-2 rounded-xl border border-amber-300/70 bg-amber-50 p-3 text-sm leading-relaxed text-amber-950 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100"
            >
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <DocumentReviewActions
                          ...
                          onReview={onReview}
                          labels={{
                            resume: t("reviewEditCv"),
                            cover: t("reviewEditCover"),
                            openingResume: t("openingCv"),
                            openingCover: t("openingCover"),
                            unavailable: t("reviewUnavailable"),
                            openResume: t("openCvPdf"),
                            openCover: t("openCoverPdf"),
                          }}
                        />
                      </div>
                    </div>
              ...
              <ul className="space-y-2">
                {state.succeededItems.map((item) => (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <FileText
                        className="mt-0.5 h-4 w-4 shrink-0 text-brand-emerald-text"
                        aria-hidden
                      />
...
function DocumentReviewActions({
  ...
  loading: ApplicationReviewLoad | null;
  onReview: (
    applicationId: string,
    jobId: string,
    target: "resume" | "cover",
  ...
  if (!applicationId) {
    const downloads = [
      {
        ...
        url: artifacts.resumePdfUrl,
        label: labels.openResume,
      },
      {
        target: "cover" as const,
        url: artifacts.coverPdfUrl,
        label: labels.openCover,
      },
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {downloads.map((download) => (
            <Button
              ...
              variant="outline"
              size="touch"
              className="min-h-11 border-border bg-background px-3 text-xs font-semibold text-foreground/80 hover:border-brand-emerald-300 hover:bg-brand-emerald-50 hover:text-brand-emerald-text focus-visible:ring-brand-emerald-600/40 dark:hover:bg-brand-emerald-500/10"
            >
    ...
    return (
      <p className="mt-2 text-xs text-muted-foreground">{labels.unavailable}</p>
    );
  }

  return (
    <div className="mt-2 flex flex-wrap gap-2">
      {(["resume", "cover"] as const).map((target) => {
        const pending =
          loading?.applicationId === applicationId && loading.target === target;
        const label = target === "resume" ? labels.resume : labels.cover;
        const pendingLabel =
          target === "resume" ? labels.openingResume : labels.openingCover;
        return (
          <Button
            key={target}
            type="button"
            ...
            onClick={() => void onReview(applicationId, jobId, target)}
            className="min-h-11 border-brand-emerald-200 bg-background px-3 text-xs font-semibold text-brand-emerald-text hover:border-brand-emerald-300 hover:bg-brand-emerald-50 focus-visible:ring-brand-emerald-600/40 dark:border-brand-emerald-500/30 dark:hover:bg-brand-emerald-500/10"
          >
            {pending ? (
              <Loader2 className="motion-safe:animate-spin" aria-hidden />
            ) : (
