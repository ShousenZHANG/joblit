...
beforeEach(() => {
  ...
  const mockFetch = vi.fn(async (input: RequestInfo, init?: RequestInit) => {
    ...
    if (url.startsWith("/api/jobs/") && (!init || init.method === "GET")) {
      return new Response(
        JSON.stringify({ id: baseJob.id, description: "Job description", updatedAt: baseJob.updatedAt }),
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    }
...
describe("JobsClient", () => {
  ...
  it("renders markdown with SaaS-style headings and lists", async () => {
    ...
    const mockFetch = vi.fn(async (input: RequestInfo, init?: RequestInit) => {
      ...
      if (
        ...
        return new Response(
          JSON.stringify({ id: baseJob.id, description: markdown, updatedAt: baseJob.updatedAt }),
          { status: 200, headers: { "Content-Type": "application/json" } },
        );
      }
      return new Response(JSON.stringify({ error: "not mocked" }), { status: 500 });
  ...
  it("shows the empty state (not a blank list) after the last visible job is deleted", async () => {
    ...
    const mockFetch = vi.fn(async (input: RequestInfo, init?: RequestInit) => {
      ...
      if (url.startsWith("/api/jobs/") && (!init || init.method === "GET")) {
        return new Response(JSON.stringify({ id: baseJob.id, description: "d", updatedAt: baseJob.updatedAt }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
      }
