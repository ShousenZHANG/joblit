...
It binds only `127.0.0.1`, creates a random 32-byte API key, atomically writes the profile-local `.env` with a current-user ACL, and advertises the opaque profile name as `API_SERVER_MODEL_NAME`. Beta sideload packages also set `API_SERVER_CORS_ORIGINS=*` so Chrome extension service workers with changing unpacked IDs can reach the authenticated loopback API; the bearer key is still required and never enters website code. It probes `/health`, authenticated `/v1/capabilities`, `/v1/models`, `/v1/toolsets`, and the non-billable `/v1/runs` CORS preflight, validates the exact Runs API routes, and rejects any enabled executable tools. It never sends a billable run. A newly generated API key appears once for pasting into Joblit; structured status and the receipt contain only its fingerprint.
