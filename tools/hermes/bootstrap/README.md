...
It binds only `127.0.0.1`, creates a random 32-byte API key, atomically writes the profile-local `.env` with a current-user ACL, and advertises the opaque profile name as `API_SERVER_MODEL_NAME`. The local Runner calls this loopback API from Node, so the bootstrap removes the retired browser CORS setting instead of exposing a wildcard origin. It probes `/health`, authenticated `/v1/capabilities`, `/v1/models`, `/v1/toolsets`, and `/v1/runs`, validates the exact Runs API routes, and rejects any enabled executable tools. It never sends a billable run. A newly generated API key appears once for configuring the Runner's local `HERMES_KEY`; structured status and the receipt contain only its fingerprint.
