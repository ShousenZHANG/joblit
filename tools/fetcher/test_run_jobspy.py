...
class FilterJobTypeTests(unittest.TestCase):
    def _df(self, rows):
        return pd.DataFrame(rows)

    def test_excludes_internship_and_contract_by_alias(self):
        df = self._df(
            [
                {"title": "Grad SWE", "job_type": "Full-time"},
                {"title": "Intern", "job_type": "Internship"},
                {"title": "Contractor", "job_type": "Contract"},
            ]
        )
        out = rj.filter_job_type(df, ["internship", "contract"])
        self.assertEqual(out["title"].tolist(), ["Grad SWE"])

    def test_falls_back_to_employment_type_column(self):
        df = self._df(
            [
                {"title": "A", "employment_type": "Permanent"},
                {"title": "B", "employment_type": "internship"},
            ]
        )
        out = rj.filter_job_type(df, ["internship"])
        self.assertEqual(out["title"].tolist(), ["A"])

    def test_no_type_column_keeps_all(self):
        df = self._df([{"title": "A"}, {"title": "B"}])
        out = rj.filter_job_type(df, ["internship"])
        self.assertEqual(len(out), 2)

    def test_empty_exclude_keeps_all(self):
        df = self._df([{"title": "A", "job_type": "Internship"}])
        self.assertEqual(len(rj.filter_job_type(df, [])), 1)


class FilterRemoteOnlyTests(unittest.TestCase):
    def test_keeps_only_is_remote_true(self):
        df = pd.DataFrame(
            [
                {"title": "A", "is_remote": True, "location": "Sydney"},
                {"title": "B", "is_remote": False, "location": "Sydney"},
            ]
        )
        out = rj.filter_remote_only(df)
        self.assertEqual(out["title"].tolist(), ["A"])

    def test_unknown_flag_uses_text_heuristic(self):
        df = pd.DataFrame(
            [
                {"title": "Remote SWE", "is_remote": None, "location": "Anywhere"},
                {"title": "Onsite SWE", "is_remote": None, "location": "Sydney"},
            ]
        )
        out = rj.filter_remote_only(df)
        self.assertEqual(out["title"].tolist(), ["Remote SWE"])

    def test_no_flag_column_uses_text(self):
        df = pd.DataFrame(
            [
                {"title": "SWE (WFH)", "location": "Sydney", "description": ""},
                {"title": "SWE", "location": "Sydney", "description": "office based"},
            ]
        )
        out = rj.filter_remote_only(df)
        self.assertEqual(out["title"].tolist(), ["SWE (WFH)"])


class FilterMinSalaryTests(unittest.TestCase):
    def test_drops_below_floor_keeps_unknown(self):
        df = pd.DataFrame(
            [
                {"title": "High", "max_amount": 150000, "interval": "yearly"},
                {"title": "Low", "max_amount": 60000, "interval": "yearly"},
                {"title": "Unknown", "max_amount": None, "interval": "yearly"},
            ]
        )
        out = rj.filter_min_salary(df, 100000)
        self.assertCountEqual(out["title"].tolist(), ["High", "Unknown"])

    def test_annualizes_hourly(self):
        df = pd.DataFrame(
            [
                {"title": "Hourly80", "max_amount": 80, "interval": "hourly"},
                {"title": "Hourly20", "max_amount": 20, "interval": "hourly"},
            ]
        )
        out = rj.filter_min_salary(df, 100000)  # 80*2080=166k keep, 20*2080=41k drop
        self.assertEqual(out["title"].tolist(), ["Hourly80"])

    def test_no_salary_column_keeps_all(self):
        df = pd.DataFrame([{"title": "A"}, {"title": "B"}])
        self.assertEqual(len(rj.filter_min_salary(df, 100000)), 2)

    def test_zero_floor_noop(self):
        df = pd.DataFrame([{"title": "A", "max_amount": 1, "interval": "yearly"}])
        self.assertEqual(len(rj.filter_min_salary(df, 0)), 1)
